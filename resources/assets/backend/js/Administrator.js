export default class Administrator {
    ...
    static usernameGenerator() {
        ...
        $('input[name="students"]').on('ifChanged', function(){
            if($(this).is(':checked')) {
                $('#classNsection').removeClass('hidden');
            }else{
                $('#classNsection').addClass('hidden');
            }
        });
        ...
        $('input[name="sms"]').on('ifChanged', function () {
           showOrhideMessage();
        });
		...
		function showOrhideMessage(){
			if($('input[name="sms"]').prop('checked')){
                $('#smsContent').removeClass('hidden');
                $('#sms_tags').closest('.form-group').removeClass('hidden');
			}else{
                $('#smsContent').addClass('hidden');
                $('#sms_tags').closest('.form-group').addClass('hidden');
			}
		}
    }
    ...
    static setidcardTemplateData  (data) {
        ...
        static getHomeworkSubmissionForm() {
            $(document).on('click', '.submitHomework', function() {
                ...
                if(hwID != '') {
                    ...
                    $.ajax({
                        type: 'GET',
                        url: window.submissionForm,
                        data: { homeworkID: hwID },
                        success:function(submitForm) {
                            $('.modal-body').empty();
                            $('.modal-title').text('Homework Submission');
                            $('.modal-body').html(submitForm.html);
                            Generic.loaderStop();
                            $('#modalHomeworkSubmissionForm').modal('show');
                        }
                    });
                }
            });
        ...
        static getHomeworkSubmissions() {
            $(document).on('click', '.homeworkSubmissions', function() {
                var hwID = $(this).attr('data-hwID');
                if(hwID != '') {
                    Generic.loaderStart();
                    $.ajax({
                        type: 'GET',
                        url: window.homeworkSubmissions,
                        data: { homeworkID: hwID },
                        success:function(submissions) {
                            $('.modal-body').empty();
                            $('.modal-title').text('Homework Submissions');
                            $('.modal-body').html(submissions.html);
                            Generic.loaderStop();
                            $('#modalHomeworkSubmissionForm').modal('show');
                        }
                    });
        ...
        static updateSubmissionStatus() {
            $(document).on('click', '.statusUpdate', function() {
                var subID = $(this).attr('data-hwSubID');
                var statusVal = $(this).val();
                if(subID != '') {
                    Generic.loaderStart();
                    axios.post(window.updateSubmissionStatus, { submissionID: subID, status: statusVal })
                    .then((response) => {
                        if(response.data.status == 'success') {
                            $('#hwStatus-'+response.data.homeworkID).text(response.data.hwStatus);
                        }
    ...
    static templateStudyCertificatePreview() {
        $('.btnStudyCertificatePreview').click(function () {
            //clear preview content
            $('.modal-body').empty();
            $('.modal-body').append(' <iframe id="idFrame" style="width: 100%; height: 950px;" src="" frameborder="0">\n' +
                '                    </iframe>');

            let pk = $(this).attr('data-id');
            Generic.loaderStart();
            Administrator.getTemplateStudyCertificateData(pk)
                .then((response) => {
                    let template = response.data;
                    // console.log(template);
                    if (Object.keys(template).length) {
                        let templateSample = window.templateUrl + ".html";
                        //now feed data to iframe
                        $('#idFrame').attr('src', templateSample);
                        $('iframe').on('load', function() {
                            // console.log('iframe load');
                            //now fill colors
                            Administrator.setStudyCertificateTemplateData(template);
                        });

                        //now show the modal
                        Generic.loaderStop();
                        $('#modalStudyCertificatePreview').modal('show');
                    }
                    else {
                        toastr.warning("Template content not found!");
                    }
    ...
    static getTemplateStudyCertificateData(pk) {
        //get template data
        let getUrl = window.templateGetURL + "?pk=" + pk;
        return axios.get(getUrl)
            .then((response) => {
    ...
    static setStudyCertificateTemplateData  (data) {
        // console.log(data);
        if(data.bg_color) {
            $('#idFrame').contents().find('body div.card').css('background-color', data.bg_color);
        }
        ...
        if(data.BG_image) {
            let logoBG_image = "data:image/png;base64,"+data.BG_image;
            $('#idFrame').contents().find('body div.BG_image').css('background-image', 'url('+logoBG_image+')');
        }
        if(data.logo) {
            let logoDataImage = "data:image/png;base64,"+data.logo;
            $('#idFrame').contents().find('body div.logo img').attr('src', logoDataImage);
    ...
    static templateStudyCertificateInit() {
        var templateMainUrl = window.templateUrl;
        var logoDataImage = window.logoString || "";
        var signatureDataImage = window.signatureString || "";
        var BG_imageData = window.BG_imageString || "";

        $('.select2').select2();

        var fullTemplateUrl = templateMainUrl + ".html";
        $('#idFrame').attr('src', fullTemplateUrl);
        ...
        $('#BG_image').change(function(){
            ...
            if (input.files && input.files[0]&& (ext == "png" || ext == "jpeg" || ext == "jpg"))
            {
                var reader = new FileReader();
                reader.onload = function (e) {
                    $('#idFrame').contents().find('body div.BG_image').css('background-image', 'url('+e.target.result+')');
                };
                reader.readAsDataURL(input.files[0]);
            }
            else
            {
                if(BG_imageData) {
                    $('#idFrame').contents().find('body div.BG_image').css('background-image', 'url('+BG_imageData+')');
                }
            }
        });
        $('#logoUp').change(function(){
    ...
    static templateAdmitCardPreview() {
        $('.btnAdmitCardPreview').click(function () {
            //clear preview content
            $('.modal-body').empty();
            $('.modal-body').append(' <iframe id="idFrame" style="width: 100%; height: 950px;" src="" frameborder="0">\n' +
                '                    </iframe>');
            ...
            let pk = $(this).attr('data-id');
            Generic.loaderStart();
            Administrator.getTemplateAdmitCardData(pk)
                .then((response) => {
                    let template = response.data;
                    // console.log(template);
                    if (Object.keys(template).length) {
                        ...
                        $('iframe').on('load', function() {
                            // console.log('iframe load');
                            //now fill colors
                            Administrator.setAdmitCardTemplateData(template);
                        });

                        //now show the modal
                        Generic.loaderStop();
                        $('#modalAdmitCardPreview').modal('show');
                    }
                    else {
                        toastr.warning("Template content not found!");
                    }
    ...
    static getTemplateAdmitCardData(pk) {
        //get template data
        let getUrl = window.templateGetURL + "?pk=" + pk;
        return axios.get(getUrl)
            .then((response) => {
    ...
    static setAdmitCardTemplateData  (data) {
        // console.log(data);
        if(data.bg_color) {
            $('#idFrame').contents().find('body div.card').css('background-color', data.bg_color);
        }
        ...
        if(data.exam_note) {
            $('#idFrame').contents().find('body .examNote').html(data.exam_note);
        }
    ...
    static templateAdmitCardInit() {
        var templateMainUrl = window.templateUrl;
        var logoDataImage = window.logoString || "";
        var examNote = window.examNote || "";
        ...
        $('#exam_note').on('change keyup paste', function() {
            $('#idFrame').contents().find('body .examNote').html($(this).val());
        });
