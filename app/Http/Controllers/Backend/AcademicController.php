...
//            }
            ...
            // Updated code details
                ...
                foreach ($subjects as $subject) {
                    $examRule = ExamRule::where('exam_id', $exam_id)
                                ...
                                ->where('combine_subject_id', '<>', null)
                                ->pluck('combine_subject_id')->toArray();

                    if( count($examRule) > 0 )
                        $final_ids[] = implode(',', $examRule);
                }

                $com_data = [];
                if( count($final_ids) > 0  ) {
                    $com_data = implode(',', $final_ids);
                }
                ...
                if( count($final_ids) > 0 )
                    $subjects->whereNotIn('id', explode(',', $com_data));
        ...
        $class_id = $request->query->get('class',0);
        $subjects = Subject::select('*')->iclass($class_id)->with('teacher')->with('class')->orderBy('name', 'asc')->get();
        $classes = IClass::where('status', AppHelper::ACTIVE)->orderBy('order', 'asc')
            ->pluck('name', 'id');
        $iclass = $class_id;
